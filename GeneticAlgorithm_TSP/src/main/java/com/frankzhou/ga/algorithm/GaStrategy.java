...
public class GaStrategy {
    ...
    public static int pop_number = 100;
    ...
    public static Chromosome crossover(Chromosome p_one,Chromosome p_two)//OX交叉方式，返回一个新的Chromosome，交换亲代即可获得两个子代
    {
    ...
    public static Chromosome mutation(Chromosome chromosome)// 突变，也是Ls,提高才能停止迭代
    {
    ...
    public static Chromosome neighborhood_move(Chromosome chromosome)
    {
        Random r = new Random();
        int i = r.nextInt(Conf.N) + 1;
        int j = r.nextInt(Conf.N) + 1;
    ...
    public static Chromosome[] selection(Chromosome[] chromosomes)//选择，保留优秀个体，二进制锦标赛选择
    {
    ...
    public static Chromosome getbest(Chromosome[] chromosomes)
    {
    ...
    public static double get_mean(Chromosome[] chromosomes)
    {
        double ans = 0;
    ...
    public static Chromosome genetic_algoritm()//遗传算法主流程
    {
        Chromosome best = new Chromosome();
        double min = 999999;
        Chromosome[] parents = GaStrategy.initialize();//初始化
        for(int i=1;i<= 1000;i++)
        {
            Chromosome[] mid = GaStrategy.selection(parents);//选择
            ...
            if(min>GaStrategy.getbest(parents).fitness)
            {
                best = GaStrategy.getbest(parents);
                min = best.fitness;
            }
